...
<div class="mb-6">
	<a href="/dokumente" class="text-sm text-red-700 hover:underline">&larr; Zurück</a>
</div>
...
<form method="POST" enctype="multipart/form-data" class="max-w-lg space-y-4">
	...
	<div>
		<label for="file" class="block text-sm font-medium text-gray-700">Datei * (max. 10 MB)</label>
		<input id="file" name="file" type="file" required
			accept=".pdf,.doc,.docx,.jpg,.jpeg,.png,.gif,.webp"
			class="mt-1 block w-full text-sm text-gray-500 file:mr-4 file:rounded-lg file:border-0 file:bg-red-50 file:px-4 file:py-2 file:text-sm file:font-medium file:text-red-700 hover:file:bg-red-100" />
		<p class="mt-1 text-xs text-gray-400">Erlaubt: PDF, DOCX, JPG, PNG, GIF, WebP</p>
	</div>
