...
<svelte:head>
	<title>Neue Veranstaltung - Förderverein FF</title>
</svelte:head>
...
<h1 class="mb-6 text-2xl font-bold text-gray-900">Neue Veranstaltung</h1>
...
<form method="POST" class="max-w-lg space-y-4">
	...
	<div class="grid gap-4 sm:grid-cols-2">
		...
		<div>
			<label for="time" class="block text-sm font-medium text-gray-700">Uhrzeit</label>
			<input id="time" name="time" type="time" value={form?.time ?? ''}
				class="mt-1 block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-red-500 focus:ring-1 focus:ring-red-500 focus:outline-none" />
		</div>
	</div>
