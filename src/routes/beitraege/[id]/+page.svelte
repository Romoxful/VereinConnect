...
<h1 class="mb-6 text-2xl font-bold text-gray-900 dark:text-slate-100">
...
{#if isVorstand}
	...
	<form method="POST" action="?/update" class="max-w-lg space-y-4">
		...
		<div>
			<label for="paidDate" class="block text-sm font-medium text-gray-700 dark:text-slate-300">Bezahlt am</label>
	...
	<div class="mt-8 border-t border-gray-200 dark:border-slate-700 pt-6">
		...
		{:else}
			<div class="rounded-lg bg-red-50 dark:bg-red-950 p-4">
				<p class="mb-3 text-sm text-red-700 dark:text-red-400">Beitrag wirklich löschen?</p>
...
{:else}
	...
	<div class="max-w-lg space-y-3 rounded-lg bg-white dark:bg-slate-800 p-6 shadow">
		<div class="grid gap-3 sm:grid-cols-2">
			<div>
				<p class="text-xs text-gray-500 dark:text-slate-400">Mitglied</p>
		...
		<div class="grid gap-3 sm:grid-cols-2">
			<div>
				<p class="text-xs text-gray-500 dark:text-slate-400">Betrag</p>
				<p>{d.amount} €</p>
			</div>
			<div>
				<p class="text-xs text-gray-500 dark:text-slate-400">Jahr</p>
				<p>{d.year}</p>
			</div>
		</div>
		<div class="grid gap-3 sm:grid-cols-2">
			<div>
				<p class="text-xs text-gray-500 dark:text-slate-400">Fällig am</p>
				<p>{new Date(d.dueDate).toLocaleDateString('de-DE')}</p>
			</div>
			{#if d.paidDate}
				<div>
					<p class="text-xs text-gray-500 dark:text-slate-400">Bezahlt am</p>
