...
<svelte:head>
	<title>Neues Mitglied - Förderverein FF</title>
</svelte:head>
...
<h1 class="mb-6 text-2xl font-bold text-gray-900 dark:text-slate-100">Neues Mitglied</h1>
...
<form method="POST" class="max-w-lg space-y-4">
	...
	<div class="grid gap-4 sm:grid-cols-2">
		<div>
			...
			<input id="memberSince" name="memberSince" type="date" required value={form?.memberSince ?? ''}
				class="mt-1 block w-full rounded-lg border border-gray-300 dark:border-slate-600 px-3 py-2 text-sm shadow-sm focus:border-red-500 focus:ring-1 focus:ring-red-500 focus:outline-none" />
		</div>
		<div>
			...
			<select id="status" name="status"
				class="mt-1 block w-full rounded-lg border border-gray-300 dark:border-slate-600 px-3 py-2 text-sm shadow-sm focus:border-red-500 focus:ring-1 focus:ring-red-500 focus:outline-none">
				<option value="aktiv" selected={form?.status === 'aktiv' || !form?.status}>Aktiv</option>
				<option value="inaktiv" selected={form?.status === 'inaktiv'}>Inaktiv</option>
				<option value="ausgetreten" selected={form?.status === 'ausgetreten'}>Ausgetreten</option>
				<option value="beantragt" selected={form?.status === 'beantragt'}>Beantragt</option>
				<option value="abgelehnt" selected={form?.status === 'abgelehnt'}>Abgelehnt</option>
			</select>
		</div>
	</div>
