...
{#if editing}
	<h1 class="mb-6 text-2xl font-bold text-gray-900">Protokoll bearbeiten</h1>
...
{:else}
	<div class="max-w-2xl">
		<div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
			<h1 class="text-2xl font-bold text-gray-900">{data.protocol.title}</h1>
		...
		<div class="rounded-lg bg-white p-6 shadow">
			<div class="mb-4 grid gap-4 border-b pb-4 sm:grid-cols-3">
				<div>
					<p class="text-xs font-medium uppercase text-gray-400">Datum</p>
					<p class="text-sm text-gray-900">{new Date(data.protocol.date).toLocaleDateString('de-DE')}</p>
				</div>
				<div>
					<p class="text-xs font-medium uppercase text-gray-400">Teilnehmer</p>
					<p class="text-sm text-gray-900">{data.protocol.attendees}</p>
				</div>
				<div>
					<p class="text-xs font-medium uppercase text-gray-400">Erstellt von</p>
					<p class="text-sm text-gray-900">{data.protocol.creatorName ?? '–'}</p>
				</div>
			</div>
